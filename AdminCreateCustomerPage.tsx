...
export default function AdminCreateCustomerPage(){
  ...
  return <main className="admin-shell"><AdminNavigation/><section className="admin-main">
    ...
    {success&&<div className="admin-notice"><CheckCircle2 size={18}/><div><strong>Customer created successfully</strong><span>{success.email} · Account ending {success.account_number_last4||'—'} · Opening balance {money(success.opening_balance)}</span></div></div>}
    <div className="admin-grid">
      ...
      <section className="admin-panel"><div className="panel-head"><div><h2>Account setup</h2><p className="admin-header-subtitle">The opening amount is posted through the existing double-entry ledger; no historical customer transactions are copied.</p></div></div>
        ...
        <div className="admin-notice"><strong>LEDGER CONTROL</strong><span>Account creation uses the existing account bootstrap trigger and opening-balance journal. The new customer starts with a fresh ledger; the old synthetic 4–5 year history is not copied.</span></div>
