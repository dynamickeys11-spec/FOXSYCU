...
const functionError=async(e:any,fallback:string)=>{
 if(e?.context?.json){try{const body=await e.context.json();if(body?.error)return String(body.error);if(body?.message)return String(body.message)}catch{}}
...
export default function TransferAuthorizationPanel({movementId,amount,reference,onComplete,onBack}:Props){
 ...
 const requestToken=async()=>{if(!/^\d{4,6}$/.test(pin))return setError('Enter your 4–6 digit Transfer PIN.');setBusy(true);setError('');try{const{data,error:e}=await supabase.functions.invoke('transfer-security',{body:{action:'send_token',movement_id:movementId,pin}});if(e)throw e;if(data?.error)throw new Error(data.error);setExpiresAt(data.expires_at);setRemaining(secondsLeft(data.expires_at));setStage('token')}catch(e:any){setError(await functionError(e,'Unable to start authorization.'))}finally{setBusy(false)}}
 const authorize=async()=>{if(remaining<=0)return setError('This security code has expired. Request a new one.');if(!/^\d{8}$/.test(token))return setError('Enter the 8-digit security code.');setBusy(true);setError('');try{const{data,error:e}=await supabase.functions.invoke('transfer-security',{body:{action:'authorize',movement_id:movementId,pin,token}});if(e)throw e;if(data?.error)throw new Error(data.error);onComplete(data.result)}catch(e:any){setError(await functionError(e,'Unable to authorize this transfer.'))}finally{setBusy(false)}}
