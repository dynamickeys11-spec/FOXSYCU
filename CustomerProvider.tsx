...
/**
 * Bootstrap customer-owned records only when they are genuinely missing.
 * Existing ledger rows and account balances are NEVER rebuilt or restored here.
 */
async function ensureCustomer(userId: string) {
  ...
  // Seed the synthetic history once, only for a brand-new account with an empty ledger.
  // Never compare seed count to the live ledger and never delete/recreate transactions.
...
}, [session?.user?.id]); const value = useMemo(() => ({ ...data, loading, session, refresh }), [data, loading, session]); return <CustomerContext.Provider value={value}>{children}</CustomerContext.Provider> }
export function useCustomerData() { const value = useContext(CustomerContext); if (!value) throw new Error('useCustomerData must be used within CustomerProvider'); return value }
