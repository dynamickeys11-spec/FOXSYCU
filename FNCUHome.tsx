...
export default function FNCUHome(){
 ...
 const actions=[{label:'Transfer',icon:MoveRight,to:'/transfers'},{label:'Deposit',icon:Plus,to:'/deposit'},{label:'Pay a person',icon:Send,to:'/transfers?mode=p2p'},{label:'Pay a bill',icon:Receipt,to:'/bills'},{label:'Messages',icon:MessageSquare,to:'/messages'}]
 ...
 return <BankingShell>
   <div className="fh-reference-home">
     ...
     <section className="fh-accounts"><div className="fh-section-head"><div><span>SAVINGS</span><h2>Savings balance</h2></div><NavLink to="/transactions">View activity</NavLink></div><div className="fh-account-row"><span className="fh-account-icon"><WalletCards size={17}/></span><span><b>{vaults.length?`${vaults.length} savings goal${vaults.length===1?'':'s'}`:'Savings'}</b><small>USD · total savings</small></span><strong>{visible?money(savings):'••••'}</strong></div></section>
     <section className="fh-activity"><div className="fh-section-head"><div><span>RECENT ACTIVITY</span><h2>Transactions</h2></div><NavLink to="/transactions">View all</NavLink></div><div className="fh-activity-list">{recent.map((t:any)=><button className="fh-activity-row" key={t.id} onClick={()=>navigate('/transactions')}><span className={`fh-tx-badge ${t.amount>=0?'in':'out'}`}>{t.amount>=0?<ArrowDownLeft size={16}/>:<ArrowUpRight size={16}/>}</span><span className="fh-tx-main"><b>{t.description||t.counterparty||'Account activity'}</b><small>{shortDate(t.date||t.created_at||'')} · {t.account_name||'Checking'}</small></span><strong className={t.amount>=0?'positive':''}>{t.amount>=0?'+':''}{money(t.amount)}</strong></button>)}{!recent.length&&<div className="fh-empty">No transaction activity.</div>}</div></section>
   </div>
 </BankingShell>
}
