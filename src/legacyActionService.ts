...
export async function setCardStatus(locked: boolean) {
  ...
  const result = existing
    ...
    : await supabase.from('card_controls').insert({ user_id: userId, account_id: account.id, last4: '4821', online_purchases: true, contactless: true, atm_withdrawals: true, digital_wallet: false, ...values })
...
export async function createSupportCase(topic: SupportTopic, detail?: string) {
  const userId = await currentUserId()
  if (!userId) throw new Error('You must be signed in.')
  const labels: Record<SupportTopic, string> = {
    transaction: 'Transaction issue',
    'replace-card': 'Replace my card',
    'report-card': 'Report a card issue',
  ...
  const { error } = await supabase.from('support_cases').insert({ user_id: userId, case_number: caseNumber, subject: detail?.trim() ? `${labels[topic]} — ${detail.trim()}` : labels[topic], status: 'open', priority: topic === 'report-card' ? 'high' : 'normal' })
  if (error) throw new Error(error.message)
