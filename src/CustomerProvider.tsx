...
async function ensureCustomer(userId: string) {
  ...
  else if (account.account_name === 'FOXSYCU Private Checking' || !String(account.account_name || '').trim()) { const renamed = await supabase.from('accounts').update({ account_name: accountName }).eq('id', account.id).select('*').single(); if (renamed.error) throw renamed.error; account = renamed.data }
  ...
  if (!transactionCount) {
    const restored = await supabase.from('accounts').update({ available_balance: CANONICAL_ACCOUNT.targetBalance, posted_balance: CANONICAL_ACCOUNT.targetBalance, pending_balance: CANONICAL_ACCOUNT.pendingAmount }).eq('id', account!.id).select('*').single()
    if (restored.error) throw restored.error
    account = restored.data
    for (let i = 0; i < universe.length; i += 100) { const result = await supabase.from('transactions').insert(universe.slice(i, i + 100).map(t => dbTransaction(t, userId, account!.id))); if (result.error) throw result.error }
  }
