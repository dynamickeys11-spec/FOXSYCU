...
export function CustomerProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (!session?.user) return; const uid = session.user.id; let timer: ReturnType<typeof setTimeout> | null = null; const tick = async () => { await refresh(); timer = setTimeout(tick, 15000) }; timer = setTimeout(tick, 15000); return () => { if (timer) clearTimeout(timer) } }, [session?.user?.id])
