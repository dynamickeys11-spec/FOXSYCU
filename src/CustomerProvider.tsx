...
function dbTransaction(t: Transaction, userId: string, accountId: string) {
  return {
    ...
    transaction_type: t.type ?? 'TRANSFER',
    ...
    memo: t.memo,
    initiated_at: t.initiatedAt ?? new Date().toISOString(),
    effective_date: new Date(t.effectiveDate ?? t.date).toISOString().slice(0, 10),
    posted_at: t.postedAt ? new Date(t.postedAt).toISOString() : null,
    available_balance_after: t.availableBalanceAfter,
    posted_balance_after: t.postedBalanceAfter,
    metadata: { ...(t.metadata ?? {}), synthetic: true },
  }
}
...
async function ensureCustomer(userId: string) {
  ...
  if (!account) {
    const created = await supabase.from('accounts').insert({
      user_id: userId,
      account_type: CANONICAL_ACCOUNT.accountType,
      account_name: CANONICAL_ACCOUNT.accountName,
      currency: CANONICAL_ACCOUNT.currency,
      account_number_last4: CANONICAL_ACCOUNT.last4,
      status: 'active',
      available_balance: CANONICAL_ACCOUNT.targetBalance,
      posted_balance: CANONICAL_ACCOUNT.targetBalance,
      pending_balance: CANONICAL_ACCOUNT.pendingAmount,
    }).select('*').single()
  ...
  if (!beneficiaryCount) {
    const result = await supabase.from('beneficiaries').insert(CANONICAL_BENEFICIARIES.map(b => ({ user_id: userId, name: b.name, account_masked: b.accountMasked, beneficiary_type: b.type, status: 'active' })))
  ...
  if (!transactionCount) {
    for (let i = 0; i < universe.length; i += 100) {
      const result = await supabase.from('transactions').insert(universe.slice(i, i + 100).map(t => dbTransaction(t, userId, account!.id)))
      if (result.error) throw result.error
    }
  }
  ...
  if (!notificationCount) {
    await supabase.from('notifications').insert([
      { user_id: userId, title: 'Transfer alert', body: 'A savings transfer is pending authorization.', notification_type: 'transaction' },
      { user_id: userId, title: 'Security alert', body: 'Your recommended security controls are active.', notification_type: 'security' },
    ])
  }

  const { data: security } = await supabase.from('security_preferences').select('user_id').eq('user_id', userId).maybeSingle()
  if (!security) await supabase.from('security_preferences').insert({ user_id: userId, two_fa: true, passkey: true, alerts: true })

  const { data: card } = await supabase.from('card_controls').select('id').eq('user_id', userId).eq('account_id', account!.id).maybeSingle()
  if (!card) await supabase.from('card_controls').insert({ user_id: userId, account_id: account!.id, last4: CANONICAL_ACCOUNT.last4 })
  ...
  if (!supportCount) {
    await supabase.from('support_cases').insert([
      { user_id: userId, case_number: caseNumber('AR', 1), subject: 'Account relationship review', status: 'open', priority: 'priority' },
      { user_id: userId, case_number: caseNumber('ST', 2), subject: 'Statement request', status: 'resolved', priority: 'normal' },
    ])
  }
  ...
  if (!messageCount) {
    await supabase.from('secure_messages').insert([
      { user_id: userId, subject: 'Relationship team · Quarterly review', body: 'Your latest relationship summary is ready.' },
      { user_id: userId, subject: 'Security · Device confirmation', body: 'Your trusted-device list was updated.' },
    ])
  }
}
...
function syncDashboardLedger(rows: any[]) {
  const normalized = rows.map(row => ({
    ...
    status: row.status === 'completed' ? 'Completed' : row.status === 'pending' ? 'Pending' : row.status === 'failed' ? 'Failed' : 'Reversed',
    amount: row.direction === 'debit' ? -Number(row.amount) : Number(row.amount),
    currency: row.currency,
    direction: row.direction,
    description: row.description,
    date: row.effective_date,
    time: new Date(row.posted_at ?? row.created_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }),
  ...
  try {
    localStorage.setItem('foxsycu.transaction-universe.v2', JSON.stringify(normalized))
    localStorage.setItem('foxsycu.transactions', JSON.stringify(normalized))
    runtimeEngine.syncFromServerSnapshot(normalized)
  } catch {}
}
...
function syncLegacyCustomer(profile: any, account: any, vaults: any[]) {
  ...
  seed.account.last4 = account.account_number_last4 || CANONICAL_ACCOUNT.last4
  seed.account.availableBalance = Number(account.available_balance || 0)
  seed.account.postedBalance = Number(account.posted_balance || 0)
  seed.account.pendingBalance = Number(account.pending_balance || 0)
}
...
export function CustomerProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = async () => {
    ...
    setData({
      ...
      card: card.data,
      security: security.data ?? { two_fa: true, passkey: true, alerts: true },
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`foxsycu-customer-${uid}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'profiles', filter: `id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'accounts', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'transactions', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'savings_vaults', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'beneficiaries', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'notifications', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'card_controls', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'security_preferences', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'statements', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'support_cases', filter: `user_id=eq.${uid}` }, queueRefresh)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'secure_messages', filter: `user_id=eq.${uid}` }, queueRefresh)
      .subscribe()
    return () => {
      if (timer) clearTimeout(timer)
