...
const services = [
  ...
  ['Secure messages','Message FNCU securely','messages',Mail],
...
export default function BankingServicesPage(){
 const navigate=useNavigate(); const[active,setActive]=useState('home'); const[submitted,setSubmitted]=useState(false)
 ...
 return <BankingShell>
  <div className="bs-page">
   <header className="bs-heading"><span>FNCU DIGITAL BANKING</span><h1>{title}</h1><p>Everyday banking services, payments, documents and account support in one place.</p></header>
   {active==='home' ? <>
    <section className="bs-quick">
      ...
      <button onClick={()=>navigate('/messages')}><Mail size={17}/><span><b>Secure support</b><small>Message the credit union</small></span></button>
    </section>
    <div className="bs-grid">{services.map(([label,desc,key,Icon])=><button className="bs-service" key={key} onClick={()=>{setActive(key);setSubmitted(false)}}><span className="bs-icon"><Icon size={18}/></span><span><b>{label}</b><small>{desc}</small></span><ArrowRight size={15}/></button>)}</div>
   </> : <>
    <button className="bs-back" onClick={()=>setActive('home')}>← All banking services</button>
