...
export function AuthPage(){
  ...
  const passkey=async()=>{
    ...
    try{
      ...
      setMessage(e instanceof Error?e.message:'Passkey sign-in could not be completed.')
    }finally{setBusy(false)}
  }

  return <main className="welcome">
    <img className="welcome-logo-image" src="/fncu-logo-light.svg" alt="FNCU — First National Credit Union"/>
    ...
    <div className="welcome-actions"><button onClick={()=>{setMode('sign-up');setOpen(true)}}>Create account</button><button className="outline" onClick={()=>{setMode('sign-in');setOpen(true)}}>Sign in</button></div>
    <div className="welcome-secure"><ShieldCheck size={15}/> Your security is our priority</div>
    {open&&<div className="auth-modal"><section>
      <button className="auth-close" onClick={()=>setOpen(false)}><X/></button>
      <span className="auth-eyebrow">CUSTOMER ACCESS</span>
      <h2>{mode==='sign-in'?'Sign in to your account':'Create your customer account'}</h2>
      <form onSubmit={submit}>
        ...
        <button className="auth-submit" disabled={busy}>{busy?'Working…':mode==='sign-in'?'Sign in':'Create account'}</button>
      </form>
      ...
      <button className="auth-switch" onClick={()=>{setMode(mode==='sign-in'?'sign-up':'sign-in');setMessage('')}}>{mode==='sign-in'?'Need an account? Create one':'Already have an account? Sign in'}</button>
    </section></div>}
  </main>
}
