...
const nav=[['Overview','/',Home],['Accounts','/accounts',Wallet],['Money movement','/transfers',MoveRight],['Beneficiaries','/beneficiaries',Users],['Transactions','/transactions',Activity],['Messages','/messages',Mail],['Savings','/savings',PiggyBank],['Cards','/cards',CreditCard],['Security Center','/security',ShieldCheck],['Settings','/settings',SettingsIcon],['Profile','/profile',UserRound]] as const
function SettingsIcon(){return null}
...
export function BankingShell({children}:{children:React.ReactNode}){
 ...
 return <div className="app-shell">
   <aside className={open?'sidebar open':'sidebar'}>
     ...
     <nav>{nav.map(([label,path,Icon]:any)=><NavLink key={path} to={path} end={path==='/' } onClick={()=>setOpen(false)} className={({isActive})=>`nav-link ${isActive?'active':''}`}>{typeof Icon==='function'&&Icon({size:17})}<span>{label}</span></NavLink>)}</nav>
