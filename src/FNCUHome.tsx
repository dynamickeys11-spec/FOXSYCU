...
export default function FNCUHome(){
 ...
 const actions=[
  ...
  {label:'Deposit',icon:Plus,to:'/deposit'},
  {label:'Pay',icon:Send,to:'/bills'},
 ...
 return <BankingShell>
   <div className="fh-reference-home">
     ...
     <section className="fh-balance-card" aria-label="FNCU checking account balance"><div className="fh-card-top"><div><span>CHECKING</span><small>USD · AVAILABLE BALANCE</small></div><button onClick={()=>setVisible(v=>!v)} aria-label={visible?'Hide balance':'Show balance'}>{visible?<Eye size={17}/>:<EyeOff size={17}/>}</button></div><strong>{visible?money(balance):'••••••••'}</strong><div className="fh-account-meta"><span>Available</span><span>•••• {account?.account_number_last4||'----'}</span></div><div className="fh-account-foot"><span>Posted {visible?money(Number(account?.posted_balance||balance)):'••••'}</span><span>USD</span></div></section>
