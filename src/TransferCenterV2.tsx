...
function Card({ title, note, children }: { title: string; note?: string; children: React.ReactNode }) {
  return <section className="fb-card"><div className="fb-card-head"><div><h2>{title}</h2>{note && <p>{note}</p>}</div></div>{children}</section>
}
function Button({ children, primary = false, ...props }: { children: React.ReactNode; primary?: boolean } & React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button className={`fb-btn ${primary ? 'primary' : ''}`} {...props}>{children}</button>
}
...
export function TransferCenterV2() {
  ...
  const [rail, setRail] = useState<MoneyMovementRail>((params.get('mode') === 'wire' ? 'wire' : params.get('mode') === 'zelle' ? 'zelle_like' : params.get('mode') === 'deposit' ? 'deposit' : 'internal'))
  ...
  const submit = async () => {
    ...
    try {
      if (schedule) {
        ...
        const id = await createTransferSchedule({
          rail,
        ...
        const idempotencyKey = `foxsycu-${crypto.randomUUID()}`
        const result: any = await createAndExecuteMoneyMovement({
          idempotencyKey,
          rail,
          sourceAccountId: rail === 'deposit' ? account.id : account.id,
          beneficiaryId: rail === 'deposit' ? null : recipientId || null,
          amount: value,
          memo,
      ...
      setError(e?.message || 'The money movement could not be completed.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return <BankingShell><div className="fb-page"><header className="fb-heading"><span>MOVE MONEY</span><h1>Transfer center</h1><p>Transfer, wire, Zelle-style payments, deposits and scheduled movement with review before authorization.</p></header>
    <div className="fb-tabs">{([['internal', 'Transfer'], ['wire', 'Wire'], ['zelle_like', 'Zelle'], ['deposit', 'Deposit']] as const).map(([id, label]) => <button key={id} className={rail === id ? 'active' : ''} onClick={() => { setRail(id); setAmount(''); setSchedule(false) }}>{label}</button>)}</div>
    <div className="fb-grid"><Card title={rail === 'wire' ? 'Domestic wire' : rail === 'zelle_like' ? 'Send money' : rail === 'deposit' ? 'Deposit funds' : 'Transfer money'} note="Review recipient, amount, timing and fees before authorization.">
      ...
      <Button primary disabled={!canContinue || busy} onClick={() => { setError(''); setReview(true) }}>{busy ? <><Loader2 size={15}/> Processing</> : <>Review & authorize <ArrowRight size={15}/></>}</Button>
