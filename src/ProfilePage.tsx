...
export default function ProfilePage(){
  ...
  const removeAvatar=async()=>{if(!session?.user||!avatarUrl)return;setAvatarBusy(true);setAvatarError('');setMessage('');try{const{error:updateError}=await supabase.rpc('set_own_avatar_url',{p_avatar_url:''});if(updateError)throw updateError;setAvatarUrl('');await refresh();setMessage('Profile picture removed.')}catch(e){setAvatarError(e instanceof Error?e.message:'Unable to remove your profile picture.')}finally{setAvatarBusy(false)}}
