...
export default function ProfilePage(){
  ...
  const uploadAvatar=async(event:ChangeEvent<HTMLInputElement>)=>{const file=event.target.files?.[0];event.target.value='';if(!file||!session?.user)return;setAvatarError('');setMessage('');if(!AVATAR_TYPES.includes(file.type)){setAvatarError('Please choose a JPG, PNG, or WebP image.');return}if(file.size>MAX_AVATAR_SIZE){setAvatarError('Profile pictures must be 5 MB or smaller.');return}setAvatarBusy(true);try{const ext=file.type==='image/jpeg'?'jpg':file.type==='image/png'?'png':'webp';const path=`${session.user.id}/profile.${ext}`;const storage=supabase.storage.from('avatars');const oldPaths=['jpg','png','webp'].map(oldExt=>`${session.user.id}/profile.${oldExt}`).filter(oldPath=>oldPath!==path);const{error:removeError}=await storage.remove(oldPaths);if(removeError)throw removeError;const{error:uploadError}=await storage.upload(path,file,{upsert:false,contentType:file.type,cacheControl:'3600'});if(uploadError)throw uploadError;const{data}=storage.getPublicUrl(path);const{error:updateError}=await supabase.rpc('set_own_avatar_url',{p_avatar_url:`${data.publicUrl}?v=${Date.now()}`});if(updateError)throw updateError;setAvatarUrl(`${data.publicUrl}?v=${Date.now()}`);await refresh();setMessage('Profile picture updated.')}catch(e){setAvatarError(e instanceof Error?e.message:'Unable to upload your profile picture.')}finally{setAvatarBusy(false)}}
  const removeAvatar=async()=>{if(!session?.user||!avatarUrl)return;setAvatarBusy(true);setAvatarError('');setMessage('');try{const paths=['jpg','png','webp'].map(ext=>`${session.user.id}/profile.${ext}`);const{error:removeError}=await supabase.storage.from('avatars').remove(paths);if(removeError)throw removeError;const{error:updateError}=await supabase.rpc('set_own_avatar_url',{p_avatar_url:''});if(updateError)throw updateError;setAvatarUrl('');await refresh();setMessage('Profile picture removed.')}catch(e){setAvatarError(e instanceof Error?e.message:'Unable to remove your profile picture.')}finally{setAvatarBusy(false)}}
