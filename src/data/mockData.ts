...
function buildSyntheticLedger(): AccountLedger {
  ...
  for (let year = 2021; year <= 2026; year += 1) {
    ...
    for (let month = firstMonth; month <= lastMonth; month += 1) {
      ...
      entries.push(makeEntry({ id: `TX-${String(1000 + entries.length + 1)}`, accountId: ACCOUNT_ID, entryType: 'debit', kind: monthIndex % 3 === 0 ? 'Payment' : 'Transfer', category: monthIndex % 3 === 0 ? 'Card / merchant payment' : 'Outgoing transfer', description: monthIndex % 3 === 0 ? 'Card and merchant payments' : 'Outgoing account transfer', counterparty: monthIndex % 3 === 0 ? 'Merchant network' : 'External beneficiary', memo: 'Synthetic historical activity', date: displayDate(debitDate), time: '02:40 PM', amount: -debit, currency: 'USD', status: 'Completed', reference: `DR-${year}${String(month).padStart(2, '0')}-${String(monthIndex + 1).padStart(4, '0')}`, createdAt: debitDate }))
