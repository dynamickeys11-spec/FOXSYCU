...
function buildSyntheticLedger(): AccountLedger {
  ...
  for (let year = 2021; year <= 2026; year += 1) {
    ...
    for (let month = firstMonth; month <= lastMonth; month += 1) {
      ...
      entries.push(makeEntry({ id: `TX-${String(1000 + entries.length + 1)}`, accountId: ACCOUNT_ID, entryType: 'debit', kind: isCardMonth ? 'Payment' : monthIndex % 3 === 0 ? 'Payment' : 'Transfer', category: isCardMonth ? 'Card / merchant payment' : monthIndex % 3 === 0 ? 'Card / merchant payment' : 'Outgoing transfer', description: isCardMonth ? 'Everyday card purchase' : monthIndex % 3 === 0 ? 'Card and merchant payments' : 'Outgoing account transfer', counterparty: isCardMonth ? 'Merchant network' : monthIndex % 3 === 0 ? 'Merchant network' : 'External beneficiary', memo: 'Synthetic historical activity', date: displayDate(debitDate), time: '02:40 PM', amount: -effectiveDebit, currency: 'USD', status: 'Completed', reference: `DR-${year}${String(month).padStart(2, '0')}-${String(monthIndex + 1).padStart(4, '0')}`, createdAt: debitDate }))
      if (monthIndex % 9 === 0) { const largeDebit = 35_000 + monthIndex * 250; const transactionDate = isoDate(year, month, 12); entries.push(makeEntry({ id: `TX-${String(1000 + entries.length + 1)}`, accountId: ACCOUNT_ID, entryType: 'debit', kind: 'Transfer', category: 'Large transfer', description: 'Scheduled portfolio transfer', counterparty: 'Investment account', memo: 'Synthetic scheduled transfer', date: displayDate(transactionDate), time: '10:20 AM', amount: -largeDebit, currency: 'USD', status: 'Completed', reference: `TRF-${year}${String(month).padStart(2, '0')}-${String(monthIndex + 1).padStart(4, '0')}`, createdAt: transactionDate })) }
      if (monthIndex % 7 === 3) { const settlement = 12_500 + monthIndex * 175; const transactionDate = isoDate(year, month, 18); entries.push(makeEntry({ id: `TX-${String(1000 + entries.length + 1)}`, accountId: ACCOUNT_ID, entryType: 'credit', kind: 'Deposit', category: 'Settlement', description: 'Additional client settlement', counterparty: 'Client settlement account', memo: 'Synthetic historical activity', date: displayDate(transactionDate), time: '04:05 PM', amount: settlement, currency: 'USD', status: 'Completed', reference: `SET-${year}${String(month).padStart(2, '0')}-${String(monthIndex + 1).padStart(4, '0')}`, createdAt: transactionDate })) }
      monthIndex += 1
    }
  }
  const recent = [
