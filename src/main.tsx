...
function ProtectedAdmin(){const{session,loading}=useCustomerData();const location=useLocation();const gate=useMfaGate(session);const[adminLoading,setAdminLoading]=useState(true);const[allowed,setAllowed]=useState(false);useEffect(()=>{if(loading)return;let active=true;void(async()=>{if(!session){setAdminLoading(false);return}let{data:isAdmin}=await supabase.rpc('is_admin');if(!isAdmin){await supabase.rpc('claim_bootstrap_admin');isAdmin=(await supabase.rpc('is_admin')).data}if(active){setAllowed(Boolean(isAdmin));setAdminLoading(false)}})();return()=>{active=false}},[loading,session]);if(loading||adminLoading)return <Loading/>;if(!session)return <AuthPage/>;if(!allowed)return <main className="admin-denied"><div><div className="admin-lock">FNCU</div><h1>Admin access required</h1><p>Sign in with the authorized administrator account.</p><button onClick={()=>void supabase.auth.signOut()}>Sign out</button></div></main>;if(!gate.ready)return <Loading/>;if(gate.needs)return <MFAChallenge onVerified={()=>gate.setNeeds(false)}/>;if(location.pathname==='/admin/deposits')return <AdminDepositQueuePage/>;return <AdminPage/>}
function Root(){const location=useLocation();const navigate=useNavigate();useEffect(()=>{if(location.pathname==='/private-banking'||location.pathname==='/services')navigate('/',{replace:true})},[location.pathname,navigate]);if(location.pathname==='/reset-password')return <ResetPasswordPage/>;if(location.pathname==='/login')return <AuthPage/>;return <CustomerProvider>{location.pathname==='/profile'?<ProtectedProfile/>:location.pathname.startsWith('/admin')?<ProtectedAdmin/>:<ProtectedCustomerApp/>}</CustomerProvider>}
