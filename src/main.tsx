...
function useLegacyControlBridge(){const navigate=useNavigate();useEffect(()=>{const onClick=(event:MouseEvent)=>{const target=event.target as HTMLElement|null;const button=target?.closest('button') as HTMLButtonElement|null;if(!button)return;if(button.classList.contains('signout')){event.preventDefault();void supabase.auth.signOut();return}const menu=button.closest('.account-menu') as HTMLElement|null;if(menu){const text=button.textContent||'';navigate(/Savings/i.test(text)?'/savings':'/accounts');return}const text=(button.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();if(text==='replace card'){event.preventDefault();navigate('/support?topic=replace-card');return}if(text==='card controls'){event.preventDefault();navigate('/settings?section=cards');return}if(text==='report card'){event.preventDefault();navigate('/support?topic=report-card');return}if(text==='report a problem'){event.preventDefault();navigate('/support?topic=transaction');return}if(text==='add beneficiary'){event.preventDefault();navigate('/beneficiaries?new=1');return}if(text==='new message'){event.preventDefault();navigate('/messages?compose=1');return}if(text==='more filters'){event.preventDefault();navigate('/transactions?filters=more');return}};document.addEventListener('click',onClick);return()=>document.removeEventListener('click',onClick)},[navigate])}

function useLegacyIdentityBridge(profile:any,account:any){useEffect(()=>{if(!profile&&!account)return;const name=String(profile?.preferred_name||profile?.full_name||'FOXSYCU Customer');const initials=name.split(/\s+/).filter(Boolean).slice(0,2).map((part:string)=>part[0]?.toUpperCase()).join('')||'FC';const last4=String(account?.account_number_last4||'0000');let frame:number|null=null;let stopped=false;const replaceText=()=>{if(stopped)return;frame=null;const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);const nodes:Text[]=[];let node:Node|null;while((node=walker.nextNode()))nodes.push(node as Text);for(const text of nodes){if(!text.nodeValue)continue;const original=text.nodeValue;const value=original.replaceAll('John Doe',name).replaceAll('•••• •••• •••• 4821',`•••• •••• •••• ${last4}`).replaceAll('•••• 4821',`•••• ${last4}`).replace(/\bJD\b/g,initials);if(value!==original)text.nodeValue=value}applyLegacyAvatarBridge(profile?.avatar_url)};const schedule=()=>{if(frame===null)frame=requestAnimationFrame(replaceText)};schedule();const observer=new MutationObserver(schedule);observer.observe(document.body,{subtree:true,childList:true,characterData:true});return()=>{stopped=true;if(frame!==null)cancelAnimationFrame(frame);observer.disconnect();applyLegacyAvatarBridge(null)}},[profile,account])}
