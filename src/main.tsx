...
function ProtectedAdmin(){const{session,loading}=useCustomerData();const location=useLocation();const gate=useMfaGate(session);const[adminLoading,setAdminLoading]=useState(true);const[allowed,setAllowed]=useState(false);const[mustChange,setMustChange]=useState(false);useEffect(()=>{if(loading)return;let active=true;void(async()=>{if(!session){setAdminLoading(false);return}await supabase.rpc('claim_bootstrap_admin');const{data:isAdmin}=await supabase.rpc('is_admin');if(!isAdmin){if(active){setAllowed(false);setAdminLoading(false)}return}const{data:changeRequired}=await supabase.rpc('admin_get_password_state');if(active){setAllowed(true);setMustChange(Boolean(changeRequired));setAdminLoading(false)}})();return()=>{active=false}},[loading,session]);if(loading||adminLoading)return <Loading/>;if(!session)return <AdminLoginPage/>;if(!allowed){return <main className="admin-denied"><div><div className="admin-lock">FNCU</div><h1>Admin access required</h1><p>This account is not authorized for the FNCU administration portal.</p><button onClick={()=>void supabase.auth.signOut()}>Sign out</button></div></main>};if(mustChange)return <AdminPasswordChangePage/>;if(!gate.ready)return <Loading/>;if(gate.needs)return <MFAChallenge onVerified={()=>gate.setNeeds(false)}/>;if(location.pathname==='/admin/deposits')return <AdminDepositQueuePage/>;return <AdminPage/>}
function AdminRoute(){return <CustomerProvider><ProtectedAdmin/></CustomerProvider>}
function Root(){const location=useLocation();const navigate=useNavigate();useEffect(()=>{if(location.pathname==='/private-banking'||location.pathname==='/services')navigate('/',{replace:true})},[location.pathname,navigate]);if(location.pathname==='/reset-password')return <ResetPasswordPage/>;if(location.pathname==='/login')return <AuthPage/>;if(location.pathname.startsWith('/admin'))return <AdminRoute/>;return <CustomerProvider>{location.pathname==='/profile'?<ProtectedProfile/>:<ProtectedCustomerApp/>}</CustomerProvider>}
